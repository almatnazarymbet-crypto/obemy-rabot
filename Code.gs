...
/* Служебная функция — только для ручной выдачи прав в редакторе Apps Script.
   Выберите "authorizeMe" в выпадающем списке функций вверху и нажмите ▶ Выполнить.
   Если после этого внизу написано "Выполнение завершено" без ошибок — права выданы. */
function authorizeMe() {
  var resp = UrlFetchApp.fetch('https://www.google.com');
  Logger.log('OK, код ответа: ' + resp.getResponseCode());
}
